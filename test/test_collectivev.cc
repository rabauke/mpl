...
bool alltoallv_test() {
  const mpl::communicator &comm_world=mpl::environment::comm_world();
  int N_send=comm_world.rank()+1;
  int N_recv=(comm_world.size()*comm_world.size()+comm_world.size())/2;
  std::vector<T> v_send(N_send, comm_world.rank()+1), v_recv(N_recv), v_expected;
  for (int i=0, i_end=comm_world.size(); i<i_end; ++i)
    for (int j=0; j<i+1; ++j)
      v_expected.push_back(i+1);
  mpl::layouts<T> l_send, l_recv;
  for (int i=0, N=1, i_end=comm_world.size(); i<i_end; ++i, ++N) {
    l_send.push_back(mpl::vector_layout<T>(N_send));
    l_recv.push_back(mpl::indexed_layout<T>({{N, (N*N-N)/2}}));
  }
