...
bool reduce_scatter_func_test() {
  ...
  comm_world.reduce_scatter(add<T>(), x.data(), y.data(), l);
...
bool reduce_scatter_op_test() {
  ...
  for (int i = 1; i <= N; ++i)
    for (int j = 1; j <= i; ++j)
      x.push_back(T(j));
  std::vector<T> y(comm_world.rank() + 1);
  comm_world.reduce_scatter(mpl::plus<T>(), x.data(), y.data(), l);
  for (int i = 0; i <= comm_world.rank(); ++i)
    if (y[i] != N * (i + 1))
      return false;
  return true;
...
template<typename T>
bool reduce_scatter_lambda_test() {
  const mpl::communicator &comm_world = mpl::environment::comm_world();
  int N = comm_world.size();
  mpl::contiguous_layouts<T> l;
  for (int i = 1; i <= N; ++i)
  ...
  for (int i = 1; i <= N; ++i)
    for (int j = 1; j <= i; ++j)
      x.push_back(T(j));
  std::vector<T> y(comm_world.rank() + 1);
  comm_world.reduce_scatter([](T a, T b) { return a + b; }, x.data(), y.data(), l);
  for (int i = 0; i <= comm_world.rank(); ++i)
    if (y[i] != N * (i + 1))
      return false;
  return true;
...
BOOST_AUTO_TEST_CASE(reduce_scatter) {
  ...
  BOOST_TEST(reduce_scatter_func_test<double>());
  BOOST_TEST(reduce_scatter_op_test<double>());
  BOOST_TEST(reduce_scatter_lambda_test<double>());
}
