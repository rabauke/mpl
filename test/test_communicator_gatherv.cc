...
template<typename T>
bool gatherv_test(const std::vector<T> &send, const std::vector<T> &expected,
...
template<typename T>
bool igatherv_test(const std::vector<T> &send, const std::vector<T> &expected,
                   const mpl::layout<T> &l) {
  const mpl::communicator &comm_world{mpl::environment::comm_world()};
  if (comm_world.rank() == 0) {
    std::vector<T> v(comm_world.size() * send.size());
...
BOOST_AUTO_TEST_CASE(gatherv) {
  ...
  BOOST_TEST(gatherv_test(std::array{1, 2, 3, 4}));
  {
    const std::vector send{1, 2, 3, 4, 5, 6};
    const std::vector expected{0, 2, 3, 0, 5, 0};
    mpl::indexed_layout<int> l{{{2, 1}, {1, 4}}};
    l.resize(0, 6);
    BOOST_TEST(gatherv_test(send, expected, l));
  }

  BOOST_TEST(igatherv_test(1.0));
  BOOST_TEST(igatherv_test(std::array{1, 2, 3, 4}));
  {
    const std::vector send{1, 2, 3, 4, 5, 6};
    const std::vector expected{0, 2, 3, 0, 5, 0};
    mpl::indexed_layout<int> l{{{2, 1}, {1, 4}}};
    l.resize(0, 6);
    BOOST_TEST(igatherv_test(send, expected, l));
  }
}
