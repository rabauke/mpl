...
namespace mpl {
  ...
  public:
    ...
    explicit dist_graph_communicator(const communicator &old_comm,
                                     const source_set &ss,
                                     const dest_set &ds,
                                     bool reorder=true) {
      ...
      MPI_Dist_graph_create_adjacent(old_comm.comm,
                                     sources.size(), sources.data(), sourceweigths.data(),
                                     destinations.size(), destinations.data(), destinationweigths.data(),
                                     MPI_INFO_NULL, reorder, &comm);
    }

    dist_graph_communicator(dist_graph_communicator
                            &&other) noexcept {
      comm=other.comm;
    ...
    source_set inneighbors() const {
      ...
      MPI_Dist_graph_neighbors(comm,
                               indeg, sources.data(), sourceweigths.data(),
                               outdeg, destinations.data(), destinationweigths.data());
      source_set ss;
